...
";
    ...
    private static ImmutableArray<HandlerInfo> GetHandlerTypes(GeneratorSyntaxContext context, System.Threading.CancellationToken ct)
    {
        ...
        foreach (var iface in symbol.AllInterfaces)
        {
            ...
            switch (name)
            {
                case "Andersoft.Messaging.Abstractions.IMessageHandler<TMessage>":
                    results.Add(HandlerInfo.Handler(symbol.ToDisplayString(), args[0].ToDisplayString(), result: null));
                    break;

                case "Andersoft.Messaging.Abstractions.IMessageHandler<TMessage, TResult>":
                    results.Add(HandlerInfo.Handler(symbol.ToDisplayString(), args[0].ToDisplayString(), args[1].ToDisplayString()));
                    break;

                case "Andersoft.Messaging.Abstractions.IInterceptHandler<TMessage>":
                {
                ...
                case "Andersoft.Messaging.Abstractions.IInterceptHandler<TMessage, TResult>":
                {
    ...
    private static bool IsSagaBase(INamedTypeSymbol type) =>
        type.IsGenericType &&
        type.ConstructUnboundGenericType().ToDisplayString() == "Andersoft.Messaging.Core.Saga<>";
    ...
    private static string GenerateDispatcher(ImmutableArray<HandlerInfo> all, IReadOnlyList<SagaInfo> sagas)
    {
        ...
        sb.AppendLine("using Andersoft.Messaging.Core;");
        sb.AppendLine();
    ...
    private static string GenerateRegistration(ImmutableArray<HandlerInfo> all, IReadOnlyList<SagaInfo> sagas)
    {
        ...
        sb.AppendLine("using Andersoft.Messaging.Abstractions;");
        sb.AppendLine("using Andersoft.Messaging.Core;");
        if (hasSagas) sb.AppendLine("using Andersoft.Messaging.EntityFrameworkCore;");
        sb.AppendLine();
        sb.AppendLine("namespace Andersoft.Messaging.Abstractions;");
        sb.AppendLine();
        ...
        if (hasSagas)
        {
            ...
            if (eventTypes.Count > 0)
            {
                ...
                foreach (var ev in eventTypes)
                {
                    ...
                    sb.AppendLine($"            new Andersoft.Messaging.EntityFrameworkCore.SagaDispatcher<{ev}>(");
                    sb.AppendLine($"                sp.GetRequiredService<System.Collections.Generic.IEnumerable<Andersoft.Messaging.Core.Saga>>()));");
                }
            }
        }
