...
namespace Andersoft.CQRS.Generator;
...
namespace Andersoft.Messaging.Abstractions
{
        ...
        where TSagaState : SagaState, new()
    {
        ...
        /// </summary>
        protected internal global::System.Func<global::Andersoft.Messaging.TypedDispatcher> DispatcherFactory { get; internal set; } = null!;
        ...
        /// </summary>
        protected internal global::Andersoft.Messaging.TypedDispatcher Dispatcher => DispatcherFactory();
...
";
    ...
    private static bool IsSagaBase(INamedTypeSymbol type) =>
        type.IsGenericType &&
        type.ConstructUnboundGenericType().ToDisplayString() == "Andersoft.Messaging.Abstractions.Saga<>";
    ...
    private static string GenerateDispatcher(ImmutableArray<HandlerInfo> all, IReadOnlyList<SagaInfo> sagas)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("using Andersoft.Messaging;");
        sb.AppendLine("using Andersoft.Messaging.Abstractions;");
        sb.AppendLine();
        sb.AppendLine("namespace Andersoft.Messaging;");
        sb.AppendLine();
        ...
        if (hasSagas)
        {
            ...
            if (eventTypes.Count > 0)
            {
                ...
                foreach (var ev in eventTypes)
                {
                    ...
                    sb.AppendLine($"                sp.GetRequiredService<System.Collections.Generic.IEnumerable<Andersoft.Messaging.Abstractions.Saga>>()));");
                }
            }
        }
